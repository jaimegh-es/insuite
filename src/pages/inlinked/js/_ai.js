class QuillScribeAI {
    ...
    insertResponse() {
        if (this.lastResponse) {
            const textarea = document.getElementById('linkedin-post-editor');
            if (textarea) {
                ...
                // Inserta el texto en la posición actual del cursor o reemplaza la selección
                const start = textarea.selectionStart;
                const end = textarea.selectionEnd;
                const value = textarea.value;
                textarea.value = value.slice(0, start) + formattedText + value.slice(end);
