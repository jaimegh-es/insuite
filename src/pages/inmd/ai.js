class QuillScribeAI {
    constructor() {
        // Obtiene el contenido actual del editor Markdown
        this.getEditorMarkdown = () => {
            const editor = document.getElementById('markdown-output');
    ...
    async sendMessage() {
        ...
        const editorContent = this.getEditorMarkdown();
        ...
        try {
            ...
            const response = await this.chatEngine.chat.completions.create({
                messages: [
                    ...
                    {
                        role: "user",
                        content: `Contenido actual del editor:\n${editorContent}\n\nPregunta o instrucción:\n${message}`
                    }
                ],
                temperature: 0.7,
                max_tokens: 512
    ...
    insertResponse() {
        if (this.lastResponse) {
            // Convierte el markdown a HTML y lo inserta como HTML en el visual-editor
            if (window.insertInVisualEditor) {
                let html = this.markdownToHtml(this.lastResponse);
                // Elimina los <p> envolventes si existen, para evitar dobles saltos
                if (html.startsWith('<p>') && html.endsWith('</p>')) {
                    html = html.slice(3, -4);
                }
                window.insertInVisualEditor(html, true); // Inserta como HTML
            } else {
                alert('No se puede insertar en el editor visual.');
            }
        }
    }
